...
#include "massive-sdk/src/utils/lists.h"
#include "massive-sdk/src/CoAP/CoAP.h"
#include "board.h"
...
bool Breakout::initModem() {
  ...
  module_serial = owl_new ArduinoSeeedHwOwlSerial(&SerialModule, SerialModule_Baudrate);
  if (!module_serial) {
    GOTOERR(error_stop);
  }

  debug_serial = owl_new ArduinoSeeedUSBOwlSerial(&SerialDebugPort);
  if (!debug_serial) {
    GOTOERR(error_stop);
  }
  gnss_serial = owl_new ArduinoSeeedHwOwlSerial(&SerialGNSS, SerialGNSS_BAUDRATE);
  if (!gnss_serial) {
    GOTOERR(error_stop);
  }
  if (!(owlModem = owl_new OwlModemRN4(module_serial, debug_serial, gnss_serial))) GOTOERR(error_stop);
